...
async function getJson<T>(
  ...
  const res = await fetch(url, {
    headers: { ...baseHeaders(clientId), ...extra },
  })
...
export async function geocodeZip(
  ...
  return {
    lat: hit.latitude,
    lng: hit.longitude,
    state: hit.stateProvinceAbbreviation,
  }
}
...
export async function warehousesNear(
  lat: number,
  lng: number,
  limit = 25,
  ...
  return (data.salesLocations ?? []).filter(
    (l) => (l.subType?.code ?? '').toLowerCase() === 'warehouse',
  )
}
...
export const REGIONS: Record<
  string,
  { label: string; states: string[]; centers: Array<[number, number]> }
> = {
  northeast: {
    label: 'Northeast',
    states: [
...
export async function warehousesInRegion(
  regionKey: string,
): Promise<RegionWarehouse[]> {
  ...
  for (const [lat, lng] of region.centers) {
    const locs = await warehousesNear(lat, lng, 50).catch(
      () => [] as SalesLocation[],
    )
    for (const l of locs) {
      ...
      byNumber.set(l.salesLocationId, {
        ...
        state,
        address:
          [l.address?.line1, l.address?.city, l.address?.territory]
            .filter(Boolean)
            .join(', ') || null,
      })
    }
...
function modelFromDescription(
  longDescription: string | undefined,
): string | null {
  if (!longDescription) return null
  const m =
    longDescription.match(
      /Model\s*#?\s*:?\s*<\/strong>\s*([A-Za-z0-9._/-]+)/i,
...
export async function onlineAvailability(
  ...
  if (
    raw.includes('OUT') ||
    raw.includes('NOSTOCK') ||
    body.availableForSale === false
  )
    return 'out_of_stock'
  return 'unknown'
}
...
async function fetchWarehouseStock(
  ...
  const res = await fetch(
    ...
    {
      headers: {
        ...baseHeaders(CLIENT_IDS.inventory),
        'costco.env': 'ECOM',
...
export async function warehouseStockBatch(
  ...
  async function worker() {
    while (i < warehouseNumbers.length) {
      ...
      out.set(
        wh,
        await fetchWarehouseStock(childId, wh).catch(() => ({
  ...
  await Promise.all(
    Array.from(
      { length: Math.min(concurrency, warehouseNumbers.length) },
      worker,
    ),
  )
  return out
...
export async function seedItems(): Promise<SeedItem[]> {
  ...
  return [...byNumber].map(([costcoItemNumber, name]) => ({
    costcoItemNumber,
    name,
  }))
}
...
export async function priceAndPromos(
  childId: string,
  whsNumber: string,
  throttled = true,
  ...
  if (!res.ok)
    throw new CostcoError(
      `display-price-lite ${childId}@${whsNumber} -> HTTP ${res.status}`,
    )
