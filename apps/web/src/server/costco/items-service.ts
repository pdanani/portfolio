...
export async function checkTrackedWarehouses(itemId: number): Promise<{
  ...
  if (!childId)
    throw Object.assign(new Error('Item has no inventory id'), {
      statusCode: 400,
    })
  ...
  for (const w of tracked) {
    ...
    await db
      ...
      .onConflictDoUpdate({
        target: [
          schema.warehouseItems.warehouseId,
          schema.warehouseItems.itemId,
        ],
...
export async function scanRegion(
  ...
  if (!REGIONS[region])
    throw Object.assign(new Error(`Unknown region "${region}"`), {
      statusCode: 400,
    })
  const childId = await hydrateMetadata(itemId)
  if (!childId)
  ...
  const stock = await warehouseStockBatch(
    childId,
    list.map((w) => w.warehouseNumber),
    10,
  )

  const hits: Array<CrossWarehouseHit> = list
    .map((w) => {
      const s = stock.get(w.warehouseNumber) ?? {
