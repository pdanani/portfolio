...
const DELIVERY_LOCATIONS = [
  "729-bd", "243-wh", "1260-3pl", "1321-wm", "1477-3pl", "283-wm", "561-wm", "725-wm", "731-wm",
  "758-wm", "759-wm", "847_0-cor", "847_0-cwt", "847_0-edi", "847_0-ehs", "847_0-membership",
  "847_0-mpt", "847_0-spc", "847_0-wm", "951-wm", "952-wm", "9847-wcs",
];
...
export async function searchCatalog(
  query: string,
  opts: { offset?: number; pageSize?: number; zip?: string; state?: string; includeOutOfStock?: boolean } = {}
): Promise<SearchPage> {
  ...
  const items: SearchResultItem[] = results.map((r) => ({
    ...
    category: r.product.categories?.[0] ?? textAttr(r, "category_names") ?? null,
    imageUrl: textAttr(r, "primary_image"),
    model: textAttr(r, "model"),
    inWarehouse: (r.product.attributes?.program_types?.text ?? []).includes("InWarehouse"),
    uri: r.product.uri ?? null,
  }));
  return { items, total: data.searchResult?.totalSize ?? items.length, offset, pageSize };
}
