...
import {
  ...
  listWatches,
  removeWatch,
} from '#/server/costco/watches-service'
    ...
    handlers: {
      ...
      POST: async ({ request }) => {
        ...
        if (body.test) {
          if (!alertsConfigured()) {
            return Response.json(
              { error: 'Set NTFY_TOPIC to enable alerts' },
              { status: 400 },
            )
          }
          const ok = await sendPush(
            "Test alert from Warehouse Watch — you're all set. ✅",
            {
        ...
        if (
          !body.itemId ||
          (body.scope !== 'online' && body.scope !== 'warehouses')
        ) {
          return Response.json(
            {
